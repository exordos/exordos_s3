...
from exordos_paas_s3 import constants as c
...
class S3Instance(models.S3Instance, ua_models.InstanceWithDerivativesMixin):
    ...
    # Use `restart`, not `try-reload-or-restart`: at first boot the rustfs unit
    # is inactive (its ConditionPathExists=/etc/.../rustfs.env is unmet until the
    # control plane delivers the config). `try-reload-or-restart` is a no-op on
    # an inactive unit, so it would never start rustfs; `restart` starts it (the
    # condition now passes) and re-applies config on every subsequent change.
    OnReloadFunc = sdk_models.OnChangeShell(
        command="systemctl restart exordos-metapaas-rustfs"
    )

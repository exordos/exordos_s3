...
GENESIS_ENDPOINT = os.environ.get("GENESIS_ENDPOINT", "http://10.20.0.2:11010")
GENESIS_USERNAME = os.environ.get("GENESIS_USERNAME", "admin")
GENESIS_PASSWORD = os.environ.get("GENESIS_PASSWORD", "")
GENESIS_PROJECT_ID = os.environ.get("GENESIS_PROJECT_ID", "")

# S3aaS CP API endpoint — resolved from DNS or set explicitly
# Default: s3aas-cp.local.genesis-core.tech:8080
GENESIS_S3_CP_URL = os.environ.get(
    "GENESIS_S3_CP_URL", "http://s3aas-cp.local.genesis-core.tech:8080"
)

# S3 data endpoint (RustFS on DP node port 9000)
# If empty, derived from instance.ipsv4
GENESIS_S3_ENDPOINT = os.environ.get("GENESIS_S3_ENDPOINT", "")

# Polling defaults — CP/DP VMs take time to boot
POLL_TIMEOUT = int(os.environ.get("GENESIS_POLL_TIMEOUT", "600"))
POLL_INTERVAL = int(os.environ.get("GENESIS_POLL_INTERVAL", "15"))
...
def _get_auth_data(endpoint: str | None = None) -> dict[str, tp.Any]:
    scope = None
    if GENESIS_PROJECT_ID:
        scope = http_client.CoreIamAuthenticator.project_scope(
            sys_uuid.UUID(GENESIS_PROJECT_ID)
        )
    return dict(
        endpoint=endpoint or GENESIS_ENDPOINT,
...
def iam_rest_client() -> iam_clients.GenericAutoRefreshRESTClient:
    ...
    endpoint = f"{GENESIS_ENDPOINT.rstrip('/')}/v1/"
...
def test_user_project(iam_rest_client, test_user) -> dict:
    ...
    Also binds all genesis_s3 permissions to owner role in this project.
    """
...
def s3_endpoint(s3_instance) -> str:
    ...
    Priority:
    1. GENESIS_S3_ENDPOINT env var (explicit override)
    2. instance.ipsv4[0]:9000 (from CP API)
    """
    if GENESIS_S3_ENDPOINT:
        return GENESIS_S3_ENDPOINT
    ips = s3_instance.get("ipsv4", [])
    if not ips:
        pytest.skip("Instance has no IPs, set GENESIS_S3_ENDPOINT explicitly")
    return f"{ips[0]}:9000"

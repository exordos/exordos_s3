...
#    under the License.
"""Prepare a Genesis Core environment for integration testing.
...
4. Install the element manifest into the running Genesis Core.
...
def _install_element(
    ...
    """Install or update the element manifest in Genesis Core via CLI."""
    base_cmd = [
        "genesis",
        "-e",
        endpoint,
...
def build_parser() -> argparse.ArgumentParser:
    p = argparse.ArgumentParser(
        description="Prepare a Genesis Core environment for integration testing",
    ...
    p.add_argument(
        "--endpoint",
        default=os.environ.get("GENESIS_ENDPOINT", "http://10.20.0.2:11010"),
        help="Genesis Core API endpoint",
    )
    p.add_argument(
        "--username",
        default=os.environ.get("GENESIS_USERNAME", "admin"),
        help="Genesis Core admin username",
    )
    p.add_argument(
        "--password",
        default=os.environ.get("GENESIS_PASSWORD", ""),
        help="Genesis Core admin password",
    )
    p.add_argument(
        "--project-id",
        default=os.environ.get("GENESIS_PROJECT_ID"),
        help="Genesis Core project ID for scoped auth",
    )
    p.add_argument(
        "--repository",
        default=None,
...
def main(argv: list[str] | None = None) -> None:
    ...
    if not args.no_http_server:
        ...
        # The manifest references images as:
        #   {repository}/s3aas/{version}/images/genesis-s3.raw.gz
        # But genesis build puts them at:
        #   {output_dir}/images/genesis-s3.raw.gz
    ...
    # ------------------------------------------------------------------
    # Step 4: Install element manifest into Genesis Core
    # ------------------------------------------------------------------
    if not args.skip_install:
        _log("Step 4: Installing element manifest into Genesis Core")
    ...
    if repository_url:
        _log("  GENESIS_S3_CP_URL=http://s3aas-cp.local.genesis-core.tech:8080")
    _log(f"  GENESIS_ENDPOINT={args.endpoint}")
    _log(f"  GENESIS_USERNAME={args.username}")
    _log(f"  GENESIS_PASSWORD={args.password}")
    if args.project_id:
        _log(f"  GENESIS_PROJECT_ID={args.project_id}")
